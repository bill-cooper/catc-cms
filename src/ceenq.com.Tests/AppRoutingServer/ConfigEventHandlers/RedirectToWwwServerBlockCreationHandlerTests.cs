...
{
    [TestFixture]
    public class RedirectToWwwServerBlockCreationHandlerTests
    {
        [Test]
        public void ShouldCreateServerBlockIfAppHasCustomDomain()
        {
            ...
            var handler = new RedirectToWwwServerBlockCreationHandler();
        ...
        [Test]
        [ExpectedException(typeof(ConfigGenerationException), ExpectedMessage = "Could not generate 'redirect to www' server block.  The config context was not supplied.")]
        public void ShouldThrowExceptionIfConfigContextIsMissingApplicationObject()
        {
            var handler = new RedirectToWwwServerBlockCreationHandler();
