...
namespace ceenq.com.Layouts.Controllers {
    ...
    public class FilterController : Controller {
        ...
        public ActionResult Edit(int id, string category, string type, int filterId = -1) {
            ...
            var filter = _projectionManager.DescribeFilters().SelectMany(x => x.Descriptors).FirstOrDefault(x => x.Category.ToLower() == category.ToLower() && x.Type.ToLower() == type.ToLower());
        ...
        public ActionResult EditPost(int id, string category, string type, [DefaultValue(-1)] int filterId, FormCollection formCollection) {
            ...
            var filter = _projectionManager.DescribeFilters().SelectMany(x => x.Descriptors).Where(x => x.Category.ToLower() == category.ToLower() && x.Type.ToLower() == type.ToLower()).FirstOrDefault();
