...
namespace ceenq.com.Layouts.Providers
{
    ...
    [OrchardSuppressDependency("Orchard.Layouts.Providers.ContentFieldElementHarvester")]
    public class ContentFieldElementHarvester : Component, ElementHarvester
    {
        ...
        {
            var elementType = typeof(Orchard.Layouts.Elements.ContentField);
        ...
        private void Displaying(ElementDisplayContext context)
        {
            ...
            {
                context.ElementShape.ContentField = string.Format("[ Field : {0} ]", contentField.DisplayName);
            }
            else
            {
                var contentFieldShape = _contentFieldDisplay.Value.BuildDisplay(contentItem, contentField,
                    displayType: "Layout");
